...
class Swapstations : LocaleAwareCompatActivity(), OnMapReadyCallback, DrawerAdapter.OnItemSelectedListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener,
    LocationListener {
...
//        }
    ...
    private fun getswaps_tation() {
        val aIntent = Intent(this, commonapifetchservice::class.java)
        aIntent.putExtra(getString(R.string.intent_putextra_api_key), getString(R.string.swaps_tation))
        startService(aIntent)
    }
...
//        loader.visibility = View.GONE
        ...
        if (apiName.equals("fleet_immobilized")){
        ...
        }else if(apiName.equals(getString(R.string.swaps_tation))){
            ...
            if (response != "failed"){
                ...
                for (i in 0 until info.response!!.station!!.size){
                    ...
                    val swapmarkers = googlemap?.addMarker(MarkerOptions().position(latLng).icon(getBitmapDescriptorFromVectorDrawable(R.drawable.swapstation_pops)))!!
    ...
    private fun googleMarkerClickListener(googleMap: GoogleMap)
    {
        googleMap.setOnMarkerClickListener { marker ->
            if (marker.title == null) {
                ...
                var chap = stationItem.chap
                var meletric = stationItem.meletric
                var piam = stationItem.piam
                var commando = stationItem.commando
                if (piam != null) {
                    if (meletric != null) {
                        addd=((piam.toInt()+meletric.toInt()).toString())
                    }
                }
...
//                        .
               ...
               stationlat!!, stationlng!!,  profileimages.toString(),chap.toString(),commando.toString(),
                piam.toString(),meletric.toString(),addd.toString())
//                profileimages.toString()
    ...
    fun SwapstDialog(area:String,swapstationname:String,batteryavailable:String,lat:Double,lng:Double,profileimages:String,
                     chap:String,commando:String,piam:String,meletric:String,addd:String)
//    ,profileimages:String
    {
        ...
        var chap_count = dialog.findViewById<TextView>(R.id.chap_count)
        var commando_count = dialog.findViewById<TextView>(R.id.commando_count)
        var melectric_count = dialog.findViewById<TextView>(R.id.melectric_count)

        var chap223 = dialog.findViewById<LinearLayout>(R.id.chap223)
        var comando = dialog.findViewById<LinearLayout>(R.id.comando)
        var mastor = dialog.findViewById<LinearLayout>(R.id.mastor)
//        var addd:String = ""
...
//
        ...
        chap_count.setText(chap)
        commando_count.setText(commando)
        melectric_count.setText(meletric)

        if (chap_count.text.toString().equals("null")){
            chap223.visibility = View.GONE
        }

        if (commando_count.text.toString().equals("null")){
            comando.visibility = View.GONE
        }
           if( melectric_count.text.toString().equals("null")){
               mastor.visibility = View.GONE
           }
